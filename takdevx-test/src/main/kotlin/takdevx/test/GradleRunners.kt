@file:Suppress("CONTEXT_RECEIVERS_DEPRECATED")

package takdevx.test

import org.gradle.testkit.runner.GradleRunner
import org.junit.jupiter.api.Assumptions.assumeFalse
import java.io.File

context(ScenarioTest)
public fun File.buildRunner(requiresAndroid: Boolean = false): GradleRunner = GradleRunner
  .create()
  .withPluginClasspath()
  .withDebug(false)
  .withGradleVersion(gradleVersion)
  .withProjectDir(this)
  .apply {
    if (requiresAndroid) {
      val home = androidHome
      if (home == null) {
        assumeFalse(true, "No ANDROID_HOME supplied for an android test")
      } else {
        withEnvironment(mapOf("ANDROID_HOME" to home.absolutePath))
      }
    }
  }

context(ScenarioTest)
public fun File.runTask(
  task: String,
  requiresAndroid: Boolean = false,
  extras: List<String> = emptyList(),
): GradleRunner = buildRunner(requiresAndroid).runTask(task, extras)

public fun GradleRunner.runTask(
  task: String,
  extras: List<String> = emptyList(),
): GradleRunner = withArguments(
  listOf(
    task,
    "--configuration-cache",
    "-Pandroid.useAndroidX=true", // needed for android builds to work, unused otherwise
  ) + extras,
)
